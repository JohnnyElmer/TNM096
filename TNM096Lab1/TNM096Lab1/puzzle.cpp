...
int main() {
	...
	std::cout << "Enter 012345678 in jumbled order: " << std::endl;
    std::cout << "Hit enter between each number: " << std::endl;
...
//Send in the board you want solved
std::pair<std::string, int> Solve(std::vector<std::vector<int>> board){

    std::pair<std::string, int> results;

}

bool isGoal(std::vector<std::vector<int>> board) {
    int h1 = heuristics(board);
    if (h1 == 0) return true;
    else return false;
}

int heuristics(std::vector<std::vector<int>> board) {
    //How many are in the incorrect slot.
    int h1 = 0;
